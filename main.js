...
function gotPoses(poses) {
	if (poses.length > 0) {
		...
		scoreRightWrist = poses[0].pose.keypoints[10].score;
	}
}
...
function draw(){
	...
	if (scoreRightWrist > 0.2) {
		...
		if (!song2Playing) {
			soundFile2.play();
			document.getElementById("songPlayingLabel").innerHTML = "Song 2 Playing";
		}
	}
	...
	if(scoreLeftWrist < 0.2 && scoreRightWrist < 0.2){
		document.getElementById("songPlayingLabel").innerHTML = "No Songs Playing";
		soundFile1.stop();
		soundFile2.stop();
	}
}